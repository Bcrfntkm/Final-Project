...
class Master():
    """
    Родительский для Player класс, который отвечает за хранение основных свойств и отрисовку спрайтов.
    """
    ...
    def make_slave(self, pos):
        """
        складывает спрайты червей в массивы для удобства их дальнейшей обработки
        pos: позиция червя
        """
    ...
    def push_event(self, type, par):
        """
        складывает событие типа type в массив для их последующей обработки
        """
        self.events.append((type, par))

    def pop_event(self):
        """
        удаляет событие из массива
        """
        if len(self.events):
...
class SlaveWorm(Sprite):
    """
    Класс, обрабатывающие отображение червя на экране, включая его зеркальные отражения во время поворотов
    ...
    def update(self):
        """
        Отображение текста и выбранного оружия над червем
        """
    ...
    def draw(self, surf):
        """
        Наложение поверхности с червем на основную игровую поверхность
        """
...
class SlaveBullet(Sprite):
    """
    Класс ответственен за отрисовку пулей орудия
    """
    ...
    def draw(self, surf):
        """
        рисует пулю на поверхности surf
        """
        surf.blit(self.image, self.rect)
